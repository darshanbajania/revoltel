...
def Contact_us_view(request):
    if request.method == "POST":
        ...
        # send_mail(subject, message, 'md@revoltel.in', recipients)
            ...
            print("not submitted better luck next time")
    ...
    if request.session.has_key('cart_contents'):
        ...
        if request.method == "POST":
            ...
            if cart_product_number != None:
                cart_product = Products.objects.filter(
                    product_id=cart_product_number).first()
...
def Product_view_page(request):
    if request.session.has_key('cart_contents'):
        ...
        selected_product = Products.objects.filter(
            product_id=current_product_id).first()
        if request.method == "POST":
...
def Payment_complete_view(request):
    ...
    if bool(cart):
        for cart_item, values in cart.items():
            ...
            cart_product = Products.objects.filter(
                product_id=cart_item).first()
            cart_content_product_info = cart_product.product_name + \
                " X (" + str(values) + "): ₹" + \
                str(int(cart_product.price)*values)
            cart_contents = cart_contents + "\n" + cart_content_product_info
